			...
			if 0 < len(zs) {
				...
				for i := 0; i < len(zs); i += n {
					...
					if !z.Tangent {
						...
						hasPrev := len(zs) < i+n-1
						hasPrevPrev := len(zs) < i+n-2

						reversed := (endQ && hasPrevPrev && zs[i+n-2].AntiAligned()) || (!endQ && hasPrev && zs[i+n-1].AntiAligned())
						parallelStart := (hasPrev && zs[i+n-1].Aligned()) || reversed
